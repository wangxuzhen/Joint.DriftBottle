...
namespace Joint.Common
{
    public class WebHelper
    {
            ...
            try
            {
                using (HttpWebResponse response = GetURLResponse(url, method, postData, encoding, timeout))
                {
                    ...
                    if (encoding == null)
                    {
                        ...
                        {
                            new GZipStream(response.GetResponseStream(), CompressionMode.Decompress).CopyTo(stream, 0x2800);
                        }
                        else
                        {
                            response.GetResponseStream().CopyTo(stream, 0x2800);
                        }
                        byte[] bytes = stream.ToArray();
                        string input = Encoding.Default.GetString(bytes, 0, bytes.Length);
        ...
        public static HttpWebResponse GetURLResponse(string url, string method = "get", string postData = "", Encoding encoding = null, int timeout = 0x4e20)
        {
            if (encoding == null)
            {
                encoding = Encoding.UTF8;
