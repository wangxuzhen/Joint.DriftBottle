...
$.extend({
    ...
    validatebox: {
        ...
        rules: {
            ...
            email: {
                validator: function (value) {
                    //支持验证多个邮箱地址，多个邮箱用英文状态下的分号隔开 cly
                    var valueArr = value.split(';');
                    var result = false;
                    for (var i = 0; i < valueArr.length; i++) {
                        var valueItem = valueArr[i];
                        if (valueItem) {
                            result = /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(valueItem);
                            if (!result) {
                                //只要有一个没通过验证的话，就返回false
                                return result;
                            }
                        }
                    }
...
$.extend($.validatebox.rules, {
    ...
    Num2: {
        validator: function (value) {
            return (/^[0-9]{1,}[.]{0,}[0-9]{0,}$/).test(value);
        },
        message: '只允许输入正实数！'
    },
    Price: {
    ...
    NumPercent: {
        validator: function (value) {
            return (/^(([1-9]|[1-9][0-9])(\.\d{1,2})?|100|100.0|100.00)$/).test(value);
        },
        message: '比例应介于1-100之间,且只允许输入两位小数!'
    },
    NumPercent2: {
        validator: function (value) {
            return (/^(([0-9]|[1-9][0-9])(\.\d{1,2})?|100|100.0|100.00)$/).test(value);
        },
        message: '比例应介于0-100之间,且只允许输入两位小数!'
    },
    NotSelect: {
        validator: function (value) {
            if (value != "") {
...
function formatTime(val) {
    ...
    var html = dateTime.format("yyyy-MM-dd hh:mm:ss");
    if (html == "1970-01-01 00:00:00") {
        html = "";
    }
    return html;
}
